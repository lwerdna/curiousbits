...
from curiousbits.elf.linker import *
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        fpath = sys.argv[2]
        assert fpath.endswith('.o')
        ef = ElfFile(fpath)

        hdr_text = ef.getSectionHeader('.text')
        #print(header)
        text_contents = ef.getSectionContents('.text')

        hdr_data = ef.getSectionHeader('.data')
        #print(header)
        data_contents = ef.getSectionContents('.data')

        data = create_elf32(text_contents, data_contents, None)
        with open('linked.elf32', 'wb') as fp:
            fp.write(data)
